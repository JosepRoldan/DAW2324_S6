    ...
    <div class="flex flex-col lg:flex-row px-4 py-7 sm:px-6 lg:px-8 mx-4 sm:mx-10 md:mx-20 lg:mx-0 self-stretch">
      ...
      <div class="w-3/4">
        <div class="grid gap-x-4 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          <div
            class="relative m-7 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-lg hover:bg-gray-100">
            <a href="{{ route('products.show', ['product_id'=>$product->id]) }}"
              class="group relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl">
              <!-- Imagen principal (mostrar siempre) -->
              <img src="{{ $product->productImages->first()->thumb ?? '' }}"
                alt="Imagen principal del producto {{ $product->name }}"
                class="absolute inset-0 h-full w-full object-cover opacity-100 transition-opacity duration-300 ease-in-out {{ $product->productImages->count() > 1 ? 'group-hover:opacity-0' : '' }}" />
              <!-- Segunda imagen (mostrar al pasar el mouse, si existe) -->
              @if($product->productImages->count() > 1 && $product->productImages->skip(1)->first())
              <img src="{{ $product->productImages->skip(1)->first()->thumb ?? '' }}"
                alt="Imagen secundaria del producto {{ $product->name }}"
                class="absolute inset-0 h-full w-full object-cover opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100" />
              @endif
            </a>
