            ...
            <div class="pt-6">
                        ...
                        <ol role="list" class="flex items-center space-x-4">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ route('products.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">@lang('products.products')</a>
                                </div>
                            </li>
                            <li>
                                <div class="flex items-center">
                ...
                <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-6 lg:gap-x-8 lg:px-8">
                    ...
                    <div class="lg:grid lg:gap-y-8 col-span-1">
                        ...
                        <div class="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg cursor-pointer small-image-container {{ $loop->first ? 'highlighted' : '' }}">
                            <img src="{{ $image->original }}" alt="{{ $product->productName }}" class="h-full w-full object-cover object-center small-image" />
                        </div>
                        @endforeach
                    </div>
                    <div class="aspect-h-5 aspect-w-4 lg:aspect-h-4 lg:col-span-3 sm:overflow-hidden sm:rounded-lg zoom-container">
                        <img src="{{ $productImages->first()->original }}" alt="{{ $product->productName }}" class="h-full w-full object-cover object-center" id="mainImage" />
                    </div>
                        ...
                        <div class="mt-4 lg:row-span-3 lg:mt-0">
                            ...
                            <div class="mt-10">
                                ...
                                <div class="mt-4">
                                    ...
                                    <select id="variantSelect" name="variantSelect" class="mt-1 block w-full py-2 pl-3 pr-10 text-base text-gray-900 border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        @foreach($productVariants as $variant)
                                        <option class="text-gray-900" value="{{ $variant->variantName }}" data-variant-id="{{ $variant->id }}" data-price="{{ $variant->price + ($variant->price * $variant->benefits_margin)/100 }}">
                                            {{ $variant->name }} - {{ $variant->price + ($variant->price * $variant->benefits_margin)/100 }} €
                                        </option>
                                        @endforeach
                                    </select>
                                    <div id="priceDisplay" class="text-2xl mb-3 mt-5 font-bold tracking-tight text-gray-900 sm:text-3xl">
                                        {{ $productVariants->first()->price + ($productVariants->first()->price * $productVariants->first()->benefits_margin)/100 }} €
                                    </div>
                                </div>
                            </div>

                            <!-- Add to Bag Button -->
                            <button onclick="agregarProducto({id: '{{ $product->id }}', name: '{{ $product->name }}', image: '{{ $productImages[0]->original }}', price: '{{ $productVariants->first()->price }}', quantity: 1})" class="mt-10 flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-700 px-8 py-3 text-base font-medium text-white hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
